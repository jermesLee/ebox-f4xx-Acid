...
void Init_RA875(void)
{
	...
	WriteData(0x07);	 //     //0x14 PLLDIVN[4:0] = 4   PLLDIVM =0
	delay_ms(1);
	
	WriteCommand(0x89);
	WriteData(0x02);	// 25  //0x02 PLLDIVK[2:0] =2
	delay_ms(10);
	...
	WriteData(0x3B); //0X3B 480 //0x63 Horizontal display width(pixels) = (HDWR + 1)*8   
	...
	WriteData(0x02);//Vertical Non-Display area = (VNDR + 1)                               
	...
	Active_Window(0,480,0,272);	//设定面板分辨率为480*272 pixel
	HDIR_SEG0_SEGn();		//让屏幕显示不做旋转
	Clear_Active_Window();
...
//	Text_Background_Color(GRAY0);
	...
	PWM1_enable();
	PWM1_fnuction_sel();
	PWM1_clock_ratio(0x09);//2MHz PWM1
	PWM1_duty_cycle(240);
